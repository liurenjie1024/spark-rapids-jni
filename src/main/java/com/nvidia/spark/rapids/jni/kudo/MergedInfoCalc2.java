...
class MergedInfoCalc2 implements SchemaVisitor2 {
    ...
    private void initHasNull() {
        ...
        for (KudoTable table : kudoTables) {
            for (int i = 0; i < colNum; i++) {
                hasNull[i] = hasNull[i] | table.getHeader().hasValidityBuffer(i);
            }
        }
    }
    ...
    public void preVisitList(Schema listType) {
        ...
        if (rowCount[curColIdx] > 0) {
            ...
            offsetBufferLen = padFor64byteAlignment(rowCount[curColIdx] * Integer.BYTES);
    ...
    private class SingleTableVisitor implements SchemaVisitor2 {
        ...
        private final Deque<SliceInfo> sliceInfos = new LinkedList<>();
        private int curColIdx = 0;
