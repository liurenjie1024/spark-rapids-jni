 ...
 */
class KudoTableMerger2 extends MultiKudoTableVisitor2<Void, Void, KudoHostMergeResult> {
  ...
  protected Void doVisit(Schema primitiveType) {
    ...
    if (primitiveType.getType().hasOffsets()) {
      deserializeOffsetBuffer(offsetInfo);
      deserializeDataBuffer(primitiveType, offsetInfo, OptionalInt.empty());
    } else {
      deserializeDataBuffer(primitiveType, offsetInfo, OptionalInt.of(primitiveType.getType().getSizeInBytes()));
    }
  ...
  private void deserializeDataBuffer(Schema schema, ColumnOffsetInfo curColOffset, OptionalInt sizeInBytes) {
        ...
        if (sizeInBytes.isPresent()) {
          ...
          for (int tableIdx = 0; tableIdx < getTableSize(); tableIdx += 1) {
            ...
            if (sliceInfo.getRowCount() > 0) {
              ...
              copyDataBuffer(schema, buf, start, tableIdx, thisDataLen);
              start += thisDataLen;
            }
          }
        } else {
